...
- A **calc-ready** Property with Verdict Pass or Needs-Human is written as one **Deal input** in the **Calculator**
...
- **Calculator feed** (in scope): a Property is written as a **Deal input** iff it is **Calc-ready** *and* its Verdict is **Pass or Needs-Human**. **Reject** is never fed. (So `Needs-Human` is a handoff bucket, not a dead-end — the human confirms the missing filter data against numbers already computed.) The feed is **idempotent**: DealDesk stores the `DEALS_APP` row id on the Triage Log row and *updates* it on re-run rather than appending a duplicate.
