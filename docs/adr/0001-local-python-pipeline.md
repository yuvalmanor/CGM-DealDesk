...
- **Credential locality.** A single-operator tool holding a Google service-account key (Gmail + Sheets) and an Anthropic key is safest keeping those on the operator's own machine rather than uploading them to a cloud secrets store.
...
- The Gmail/Sheets MCP connectors wired into the development environment are **not** the production integration path — a future reader should not "wire them in" expecting that to be the intended design.
